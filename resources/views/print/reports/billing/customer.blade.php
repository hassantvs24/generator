...
@section('content')
    ...
    <div class="bg-white border-radius p-5">
        <div class="row hidden-print">
            <div class="col-xs-6 mt-10"><h6 class="m-5"><i class="icon-printer"></i> Customer Ledger</h6></div>
        ...
        <div class="row">
            <div class="col-xs-12"><h5 class="text-center no-margin">Ledger for {{$customer->name}}</h5></div>
        </div>
        <div class="row">
            <div class="col-xs-8"><b class="no-margin">Date: </b>{{$date_rang}} | <b class="no-margin">Area:</b> {{$customer->area['name']}}</div>
            ...
            <div class="col-xs-12">
                <table class="table table-bordered table-condensed">
                    ...
                    <tr>
                        ...
                        <th class="p-5 pt-0 pb-0 ">Month Name</th>
                    ...
                    @foreach($table as $row)
                        ...
                        <tr>
                            ...
                            @if($row->transactionType == 'OUT')
                                @php
                                    $billingID = $description['billingID'];
                                    $month = $row->month_name($billingID);
                                @endphp
                                <td class="p-5 pt-0 pb-0 ">{{$month ?? ''}}</td>
                            @else
                                <td class="p-5 pt-0 pb-0 ">{{$description['monthName'] ?? ''}}</td>
                            @endif
                            <td class="p-5 pt-0 pb-0 ">{{$description['transactionType']}}</td>
