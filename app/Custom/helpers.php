...
function money($amount){
    ...
    function pub_date_month($date){
        if($date == ''){
            return '';
        }else{
            return date("F d, Y", strtotime(str_replace("/", "-",  $date)));
        }
    }
    ...
    function pub_date_str($date){
        if($date == ''){
            return '';
        }else{
            return date("l, F d, Y", strtotime(str_replace("/", "-",  $date)));
        }
    }

    function single_date_range($date){
        $from = date("Y-m-d H:i:s", strtotime(str_replace("/", "-",  $date).' 00:00:00'));
        $to = date("Y-m-d H:i:s", strtotime(str_replace("/", "-",  $date).' 23:59:59'));
...
if (! function_exists('db_date')) {
    ...
    function date_range($dateRange){
         ...
         $from = date("Y-m-d", strtotime(str_replace("/", "-",  $date_range[0])));
         $to = date("Y-m-d", strtotime(str_replace("/", "-",  $date_range[1])));
    ...
    function date_time_range($dateRange){
        $date_range = explode(' - ', $dateRange);
        $from = date("Y-m-d H:i:s", strtotime(str_replace("/", "-",  $date_range[0]).' 00:00:00'));
        $to = date("Y-m-d H:i:s", strtotime(str_replace("/", "-",  $date_range[1]).' 23:59:59'));
